...
func TestStoreFixedOffsetNumber(t *testing.T) {
	...
	for _, pattern := range usagePatterns {
		...
		code := x86.StoreFixedOffsetNumber(nil, pattern.Register, pattern.Offset, pattern.Length, pattern.Number)
...
func TestStoreFixedOffsetRegister(t *testing.T) {
	...
	for _, pattern := range usagePatterns {
		...
		code := x86.StoreFixedOffsetRegister(nil, pattern.RegisterTo, pattern.Offset, pattern.Length, pattern.RegisterFrom)
		assert.DeepEqual(t, code, pattern.Code)
	}
}