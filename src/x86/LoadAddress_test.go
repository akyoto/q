...
func TestLoadAddressAbs(t *testing.T) {
	usagePatterns := []struct {
		Destination cpu.Register
		Offset      int
		Code        []byte
	}{
		{x86.R0, 0x00, []byte{0x48, 0x8D, 0x04, 0x25, 0x00, 0x00, 0x00, 0x00}},
		{x86.R0, 0x08, []byte{0x48, 0x8D, 0x04, 0x25, 0x08, 0x00, 0x00, 0x00}},
		{x86.R0, 0x10, []byte{0x48, 0x8D, 0x04, 0x25, 0x10, 0x00, 0x00, 0x00}},
		{x86.R1, 0x00, []byte{0x48, 0x8D, 0x0C, 0x25, 0x00, 0x00, 0x00, 0x00}},
		{x86.R1, 0x08, []byte{0x48, 0x8D, 0x0C, 0x25, 0x08, 0x00, 0x00, 0x00}},
		{x86.R1, 0x10, []byte{0x48, 0x8D, 0x0C, 0x25, 0x10, 0x00, 0x00, 0x00}},
	}

	for _, pattern := range usagePatterns {
		t.Logf("lea %s, [%d]", pattern.Destination, pattern.Offset)
		code := x86.LoadAddressAbs(nil, pattern.Destination, pattern.Offset)
		assert.DeepEqual(t, code, pattern.Code)
	}
}