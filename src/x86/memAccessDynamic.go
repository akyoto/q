...
func memAccessDynamic(code []byte, register cpu.Register, base cpu.Register, offset cpu.Register, scale Scale, length byte, operandLength byte, opCodePrefix []byte, opCode8 byte, opCode32 byte) []byte {
	var (
		opCode = opCode32
		mod    = AddressMemory
	)
	...
	if len(opCodePrefix) > 0 {
		code = append(code, opCodePrefix...)
	}
