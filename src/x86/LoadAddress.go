...
// LoadAddressAbs calculates the absolute address and writes the result to the destination register.
func LoadAddressAbs(code []byte, destination cpu.Register, offset int) []byte {
	code = encode2(code, AddressMemory, destination, 0b101, 0b100, Scale1, 8, 0x8D)
	return binary.LittleEndian.AppendUint32(code, uint32(offset))
}