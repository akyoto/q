...
func lintBinaryOp(binOp *ssa.BinaryOp, file *fs.File) error {
	...
	case token.Div, token.Mod, token.Shr, token.Greater, token.GreaterEqual, token.Less, token.LessEqual:
		if leftIsUnsigned && !rightIsUnsigned && rightType != types.AnyInt {
			return errors.New(&MixedSignedUnsigned{Signed: rightType.Name(), Unsigned: leftType.Name()}, file, binOp.Source)
		}

		if !leftIsUnsigned && rightIsUnsigned && leftType != types.AnyInt {
			return errors.New(&MixedSignedUnsigned{Signed: leftType.Name(), Unsigned: rightType.Name()}, file, binOp.Source)
		}
	}
	...
	if leftIsUnsigned {
		...
		if rightIsInt {
			...
			case rightInt.Int == 0:
				switch binOp.Op {
				case token.Less:
					return errors.New(AlwaysFalse, file, binOp.Source)
